...
TRANSLATIONS = {
    "zh_CN": {
        ...
        "log.non_ascii_path_warning": "⚠️ 警告：当前目录包含非英文字符（{path}），可能导致 MSYS2/Worker 启动失败。建议将程序移动到纯英文路径（如 C:\\fishtest）。",
        "log.non_ascii_path_error": "当前目录包含非英文字符，且系统未提供可用的 ASCII 短路径。无法启动 Worker，请将程序移动到纯英文路径后重试。",
    ...
    "en_US": {
        ...
        "log.non_ascii_path_warning": "⚠️ Warning: Current directory contains non-ASCII characters ({path}), which may cause MSYS2/Worker to fail. Consider moving the program to an ASCII-only path (e.g., C:\\fishtest).",
        "log.non_ascii_path_error": "The current directory contains non-ASCII characters and no usable ASCII short path is available. The Worker cannot start. Move the program to an ASCII-only path and try again.",
